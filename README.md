# 中文论文文献综述助手

一个面向中文论文文献综述前期整理的开源工具。没有 Codex 也能先用网页版，填写论文题目和文献资料，生成可复制的文献阅读整理提示词。

**免费入口：** <https://qinky1234-sys.github.io/chinese-academic-paper-skill/>

**反馈入口：**
- 总反馈：[中文论文文献综述助手使用反馈集中收集](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/2)
- 使用问题：[提交使用问题](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/new?template=usage-question.md)
- 功能建议：[提交功能需求](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/new?template=feature-request.md)
- v0.2 PDF 路线：[PDF 文献信息读取 Roadmap](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/3)

如果这个工具帮你节省了文献综述前期整理时间，欢迎在 GitHub 点一个 Star。后续会继续更新 PDF 文献信息读取、Word 导出、GB/T 7714 参考文献格式和更完整的文献核验流程。

## 项目近况（2026-06-15）

今天继续补充参考文献核验材料，把“能不能生成 GB/T 7714 格式”前面的字段检查步骤拆清楚，避免把缺字段、错字段或候选文献直接格式化成正式参考文献：

- 新增 [GB/T 7714 参考文献核验清单](./docs/gbt7714-reference-checklist.md)，用于检查文献类型、作者、题名、来源、年份、卷期页码、DOI/URL 和待核验字段。
- 新增 [AI 辅助文献综述安全使用指南](./docs/safe-ai-literature-review-workflow.md)，说明材料整理、待核验文献、研究关系、研究空白和创新点的安全写法。
- 新增 [文献综述 30 分钟自检清单](./docs/literature-review-30min-checklist.md)，用于开题、初稿或投稿前检查文献来源、综述结构、文献覆盖和创新点可行性。
- 已发布 [v0.1.13 免费网页版反馈入口与 PDF Roadmap](https://github.com/qinky1234-sys/chinese-academic-paper-skill/releases/tag/v0.1.13)。
- GitHub 仓库简介已改为“免费中文文献综述提示词生成器”，优先突出没有 Codex 也能使用。
- 目前重点收集 PDF 文献信息读取、Word 导出、GB/T 7714 参考文献格式三类需求。
- 下一步会根据真实反馈决定 v0.2 优先做 PDF 读取原型，还是先做 Word/参考文献格式支持。

上传 8 篇以上文献，或先规划 8 篇候选文献后，可以辅助整理：

- 文献综述写作框架和提示词
- 每篇文献是否写入正文的覆盖表
- 作者、题名、期刊、年份等核验提醒

没有文献也能先开始整理：它会先拆关键词、给检索式、规划公开候选文献查找路径，并把所有未确认来源标注为“待核验”。

> 不编造参考文献，不绕过数据库权限，不替代你的学术判断。它适合整理文献、搭建综述框架和生成写作提示词，不适合代写整篇论文。

## 没有 Codex？先用网页版

普通用户优先打开这个入口，不需要安装、不需要登录、不调用 API，所有输入只在浏览器本地拼接成提示词：

[免费网页版：中文论文文献综述提示词生成器](https://qinky1234-sys.github.io/chinese-academic-paper-skill/)

使用流程很简单：填写论文题目和研究方向 → 选择文献状态 → 粘贴或选择文献资料 → 复制生成的提示词到 ChatGPT、DeepSeek、豆包、通义等 AI 工具里继续生成。详细说明见：[免费网页版使用指南](./docs/web-usage-guide.md)。

如果你还不知道怎么安装 Skill，也可以打开 Markdown 版入口复制提示词：

[不安装也能先用：提示词版入口](./PROMPT_VERSION.md)

## 版本区别

| 版本 | 适合谁 | 能做什么 | 不能做什么 |
| --- | --- | --- | --- |
| 免费网页版 | 不懂 Codex、只想快速开始的普通用户 | 生成文献阅读整理提示词、检索提示词、核验清单和后续 AI 使用步骤 | 不直接调用 AI、不保存资料、不自动解析 PDF/Word、不直接生成最终论文 |
| Codex Skill 开源版 | 会用 Codex 或想看完整开源工作流的用户 | 基于用户导入文献或公开候选文献，执行矩阵、脉络、空白、创新点和综述辅助流程 | 不绕过数据库权限、不编造文献、不替代人工核验 |
| Pro 规划版 | 需要更稳定交付、导出和核验的人 | 规划支持网页内生成综述包、PDF 解析、Word 导出、GB/T 7714、字段核验和授权数据接口 | 尚未上线，现阶段只收集需求 |

## 30 秒试用

复制下面这段到 Codex 或其他 AI 工具：

```text
使用 chinese-academic-paper-assistant。
我的论文主题是【填写主题】，论文类型是【本科论文/硕士论文/期刊论文】。
请进入一键综述辅助模式，根据我上传的 8 篇以上文献整理文献覆盖简表，并生成文献综述写作框架。
如果我没有上传文献，请先规划公开候选文献查找路径，再生成待核验版综述整理提示。
所有文献都要逐篇写到，格式为“XXX学者在《xxxxx》一文中提出/指出/认为……”。
不要编造文献；公开来源候选文献必须标注为“待核验”。
```

## 快速入口

- [免费网页版：提示词生成器](https://qinky1234-sys.github.io/chinese-academic-paper-skill/)
- [免费网页版使用指南](./docs/web-usage-guide.md)
- [GB/T 7714 参考文献核验清单](./docs/gbt7714-reference-checklist.md)
- [AI 辅助文献综述安全使用指南](./docs/safe-ai-literature-review-workflow.md)
- [文献综述 30 分钟自检清单](./docs/literature-review-30min-checklist.md)
- [用户反馈入口：集中反馈 Issue](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/2)
- [用户反馈入口：使用问题](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/new?template=usage-question.md)
- [v0.2 PDF 文献信息读取设计文档](./docs/v0.2-pdf-literature-extraction.md)
- [不安装也能先用](./PROMPT_VERSION.md)
- [推广固定入口](./SHARE.md)
- [普通用户完整演示](./examples/12-free-web-user-demo.md)
- [3 分钟上手](./docs/quick-start.md)
- [真实案例：没有文献 → 8 篇候选 → 待核验综述](./examples/11-real-no-literature-to-candidates-review.md)
- [一键综述示例](./examples/10-one-click-literature-review.md)
- [复制即用提示词](./docs/copy-paste-prompts.md)
- [更新日志](./CHANGELOG.md)
- [贡献指南](./CONTRIBUTING.md)
- [MIT License](./LICENSE)

## 它解决什么问题

很多人在用 AI 写文献综述时会遇到三个问题：

- 参考文献容易被编造，作者、期刊、年份对不上。
- 综述段落像模板，只有“研究具有重要意义”，缺少具体文献脉络。
- 创新点很空泛，不知道自己到底还能从哪个角度写。

这个 skill 的工作方式相反：优先导入你已有的 CNKI、万方、维普、学校图书馆、期刊官网、Zotero、BibTeX、RIS 或 PDF 文献资料；如果你还没有文献，它会先帮你拆关键词、生成数据库检索式、推荐查找路径，并从 OpenAlex、Crossref 等公开来源补充候选文献。在候选文献元数据足够时，它可以先生成“待核验版”的文献矩阵和综述段落，等你核验后再升级为正式论文内容。

## 适合谁

- 正在写本科毕业论文、课程论文、开题报告的学生
- 正在写硕士论文、博士论文、期刊论文的研究者
- 需要整理中文文献的科研助理、课题组成员
- 想把 Zotero、CNKI、万方、维普题录转成文献矩阵的人
- 需要分析研究空白、创新点和投稿方向的人

## 不适合什么场景

- 代写整篇论文或替代作者完成学术判断
- 伪造参考文献、作者、期刊、年份、DOI 或研究结论
- 绕过 CNKI、万方、维普、学校图书馆的登录、付费、验证码或机构权限
- 承诺查重、AI 检测、期刊录用或导师认可
- 把公开来源候选文献直接当作已经核验的正式引用

## 你可以用它做什么

| 功能 | 输出 |
| --- | --- |
| 一键综述辅助 | 用户上传 8 篇以上文献，或由 skill 规划/补充候选文献后，生成综述提示词、综述草稿要求和文献覆盖简表 |
| 从零查找文献 | 在用户没有文献时，根据主题生成检索词，并从公开来源补充候选文献 |
| 初步文献综述 | 基于公开来源候选文献生成待核验版综述段落、研究脉络和研究空白，不能直接当正式引用 |
| 补充候选文献 | 当已有文献不足时，使用 OpenAlex、Crossref 等公开来源补充待核验候选文献 |
| 文献导入 | 识别 CNKI、万方、维普、Zotero、BibTeX、RIS、PDF 摘要和复制题录 |
| 首批文献主张图 | 用户首次上传文献或从公开来源找到候选文献后，列出作者+年份+一句话核心主张，按共同假设分组，标记矛盾论文，并绘制宏观背景图 |
| 文献矩阵 | 作者、年份、题名、期刊、摘要、关键词、方法、结论、局限 |
| 研究脉络 | 谁较早提出、谁可能延伸谁、哪些研究存在对比或补充关系 |
| 研究领域知识地图 | 输出核心主张、3-5 个支持柱、2-3 个争议区、1-2 个前沿问题和初学者必读论文 |
| 研究空白 | 当前文献中尚未充分解决的问题 |
| 创新点建议 | 3-5 个可写方向，并标记最接近论文、相近之处、可推进空间、推荐方法论和核验风险 |
| 三种输出模式 | 按本科论文、硕士论文、期刊论文调整文献深度、综述结构和创新点标准 |
| 综述写作 | 生成用于引言、文献综述或开题报告的中文写作框架和待核验段落 |
| 完整版文字式文献综述 | 不改变前面任何输出，只在完整流程最后追加；对所有已经找到并进入矩阵或候选表的文献逐篇对应写出“XXX学者在《xxxxx》一文中提出/指出/认为……” |
| 去 AI 化表达 | 减少模板句，保留事实、引用和学术语气 |
| 期刊推荐 | 给出中文期刊匹配思路和人工核验提醒 |
| 完整中文工作流演示 | 展示从“没有文献”到“检索策略、候选文献、矩阵、知识地图、创新点、方法论、去 AI 化综述”的端到端效果 |

## 快速开始

第一次使用建议先看：

- [3 分钟上手](./docs/quick-start.md)
- [没有文献时怎么用](./docs/no-literature-workflow.md)
- [新手使用文档](./docs/getting-started.md)
- [复制即用提示词模板](./docs/copy-paste-prompts.md)
- [常见问题 FAQ](./docs/faq.md)
- [示例输入与输出](./examples/README.md)
- [v0.1.9 完整中文工作流演示](./examples/08-full-chinese-workflow-demo.md)

最小可用提示词：

```text
使用 chinese-academic-paper-assistant。
我的论文主题是【填写主题】，论文类型是【本科论文/硕士论文/期刊论文】。
请根据我上传的 8 篇以上文献，一键整理文献覆盖表、综述写作框架和待修改综述草稿。
如果我没有上传文献，请先根据主题规划公开候选文献查找路径，再生成待核验版综述整理提示。
要求所有文献都要逐篇写到，格式为“XXX学者在《xxxxx》一文中提出/指出/认为……”，最后附文献覆盖简表。
不要编造文献；公开来源候选文献必须标注为“待核验”。
```

如果你想展开完整分析流程，可以使用这个版本：

```text
使用 chinese-academic-paper-assistant。
我的论文主题是【填写主题】，研究方向是【填写方向】。
我现在【已经有文献/还没有文献】。
如果我已经提供文献，请先整理成文献矩阵，检查字段是否完整，再分析文献共同点、研究脉络、研究空白和可选创新点。
生成创新点时，请为每个创新点标记 1-3 篇最接近论文，说明相近之处、可推进空间、推荐方法论、所需数据和核验状态。
如果这是我首次上传相关文献，或你从公开来源找到了候选文献，请先输出“作者+年份+一句话核心主张”表，再按共同假设分组，标记互相矛盾或存在张力的论文，并绘制宏观背景图；公开来源候选文献必须标注为待核验。
如果我还没有文献，请先帮我从公开来源查找候选文献，同时生成 CNKI、万方、维普、学校图书馆和期刊官网的检索策略。
如果我需要快速理解一个研究领域，请输出研究领域知识地图，包括核心主张、支持柱、争议区、前沿问题和初学者必读论文。
在候选文献元数据足够时，可以先生成“待核验版”文献矩阵和初步综述，但不要把它当成正式可引用综述。
最后请在不改变前面输出的基础上，追加一份完整版文字式文献综述，所有已经找到并进入矩阵或候选表的文献都必须逐篇对应写到，格式为“XXX学者在《xxxxx》一文中提出/指出/认为……”。如果使用公开来源候选文献，标题必须标注为“待核验版完整版文字式文献综述”。
不要编造文献；正式综述只能使用我确认过或元数据可追溯的文献。
```

如果你只想先跑通一次，可以直接复制更短版本：

```text
使用 chinese-academic-paper-assistant。我的论文主题是【填写研究方向】。我目前没有整理好的文献，请先帮我生成 CNKI、万方、维普、学校图书馆和期刊官网的检索策略，再基于可公开核验的候选文献输出待核验版文献矩阵、首批文献主张表、研究脉络、知识地图、研究空白、创新点、最接近论文和方法论建议。所有未核验文献请标记为“待核验”，不要编造参考文献，不要把待核验文献写成正式引用。
```

## 安装方式

把 skill 文件夹复制到你的 Codex skills 目录：

```powershell
mkdir $env:USERPROFILE\.codex\skills -Force
Copy-Item -Recurse .\chinese-academic-paper-assistant $env:USERPROFILE\.codex\skills\
```

然后在 Codex 中使用：

```text
使用 chinese-academic-paper-assistant，请根据我上传的 8 篇以上中文文献，一键整理文献覆盖简表和综述写作框架。
```

如果你的 Codex 支持从 GitHub 安装 skill，可以使用这个仓库路径：

```text
https://github.com/qinky1234-sys/chinese-academic-paper-skill/tree/main/chinese-academic-paper-assistant
```

## 推荐工作流

1. 如果你已经有文献，准备 8-15 条真实文献题录。
2. 如果你还没有文献，先让 skill 根据论文主题从公开来源查找候选文献，并生成 CNKI、万方、维普、学校图书馆、期刊官网的检索策略。
3. 优先导入 Zotero、BibTeX、RIS、CNKI、万方、维普、学校图书馆或期刊官网导出的资料。
4. 先生成文献矩阵，不急着写正文。
5. 检查作者、题名、期刊、年份、摘要、关键词、DOI/链接是否完整。
6. 文献不够时，再用公开来源或已配置接口补充候选文献；候选文献可以生成待核验版综述，但必须核验后再进入正式论文正文。
7. 在矩阵基础上分析共同点、研究脉络、研究空白和创新点；每个创新点都要附最接近论文和推荐方法论。
8. 最后生成去 AI 化中文综述段落；完整流程的最后再追加一份完整版文字式文献综述，逐篇覆盖所有已找到并进入矩阵或候选表的文献。

如果你不需要看矩阵、知识地图、创新点等过程，可以直接使用“一键综述模式”：上传 8 篇以上文献，或说明“没有文献，请先查找候选文献”，skill 会优先输出完整文献综述和覆盖简表。

## 三种输出模式

| 模式 | 适合场景 | 输出重点 |
| --- | --- | --- |
| 本科论文模式 | 本科毕业论文、课程论文、初次写综述 | 主题分类清楚、概念解释稳妥、创新点小而可做 |
| 硕士论文模式 | 硕士论文、开题报告、研究设计前期 | 研究脉络、理论/方法差异、研究空白、创新点可行性 |
| 期刊论文模式 | 投稿论文、论文压缩投稿、文章引言 | 问题意识、文献争议、贡献表达、期刊匹配提醒 |

使用时可以直接说：

```text
请按硕士论文模式输出：先生成文献矩阵，再分析研究脉络、研究空白和创新点，最后写一段去 AI 化中文综述。
```

## 示例效果

| 场景 | 示例 |
| --- | --- |
| 没有文献时查找候选文献并生成初步综述 | [查看示例](./examples/05-full-chinese-demo-case.md) |
| 文献不足时生成补充检索策略 | [查看示例](./examples/02-topic-to-search-strategy.md) |
| CNKI/万方/维普题录转矩阵 | [查看示例](./examples/01-imported-records-to-matrix.md) |
| 从文献矩阵生成中文综述 | [查看示例](./examples/03-matrix-to-review-paragraph.md) |
| Zotero/BibTeX/RIS 导入教程 | [查看示例](./examples/04-zotero-bibtex-ris-import.md) |
| 完整中文演示案例 | [查看示例](./examples/05-full-chinese-demo-case.md) |
| 本科/硕士/期刊论文三种输出模式 | [查看示例](./examples/06-output-modes-demo.md) |
| 创新点相近论文与方法论分析 | [查看示例](./examples/07-innovation-closest-papers-methodology.md) |
| v0.1.9 完整中文工作流演示 | [查看示例](./examples/08-full-chinese-workflow-demo.md) |
| v0.1.10 最后追加完整版文字式文献综述 | [查看示例](./examples/09-full-textual-literature-review.md) |
| v0.1.11 一键综述辅助 | [查看示例](./examples/10-one-click-literature-review.md) |
| 真实案例：没有文献 → 8 篇候选 → 待核验综述 | [查看示例](./examples/11-real-no-literature-to-candidates-review.md) |
| 普通用户完整演示：网页填写 → 复制提示词 → AI 输出检查 | [查看示例](./examples/12-free-web-user-demo.md) |

示例提示词：

```text
我的论文主题是“乡村振兴背景下基层数字治理”。我现在还没有整理好的文献，请先从公开来源查找候选文献，并生成 CNKI、万方、维普、学校图书馆和期刊官网的检索策略。候选文献元数据足够时，请先生成待核验版文献矩阵和初步综述，但不要当成正式可引用综述。
```

预期输出：

- 中文数据库检索关键词和高级检索式
- 推荐查找的数据库、期刊官网和公开来源
- 待核验候选文献
- 文献矩阵
- 文献共同点
- 谁较早提出相关问题
- 哪些研究可能存在承接关系
- 当前研究空白
- 3-5 个可选创新点，以及每个创新点对应的最接近论文和推荐方法论
- 待核验版去 AI 化中文综述段落
- 待核验版完整版文字式文献综述，所有候选文献逐篇对应出现

## 支持的导入来源

即使你暂时没有文献，也可以先输入论文主题、研究方向、关键词、学科和论文类型，让 skill 帮你查找公开候选文献、生成数据库检索路径，并产出待核验版文献综述。

- CNKI、万方、维普导出的题录或引用信息
- 学校图书馆数据库导出的文献记录
- 期刊官网公开题录
- Zotero、EndNote、NoteExpress
- BibTeX、RIS、DOI 列表
- PDF 摘要、复制的参考文献、手工整理的文献表

导入字段越完整，输出越可靠。推荐至少包含：作者、题名、期刊或来源、发表年份、摘要、关键词、DOI/URL 或数据库来源。

## 学术边界

这个 skill 不会做这些事：

- 编造参考文献、作者、期刊名、发表时间或研究结论
- 绕过 CNKI、万方、维普、学校图书馆的登录、付费、验证码或权限限制
- 虚构核心、CSSCI、CSCD、影响因子、审稿周期、版面费或录用率
- 承诺通过查重、AI 检测或期刊录用
- 把字段不完整、来源不可追溯的文献直接写进正式综述

它可以帮助你整理、分析和改写，但最终引用核验、学术判断和论文责任仍然需要由作者完成。

## 防止编造文献的规则

- 用户导入的文献优先，公开来源只作为补充线索。
- 公开来源候选文献必须标注为“待核验”。
- 不确定作者、年份、期刊、来源、DOI 或 URL 时，不能写成正式引用。
- 没有引用证据时，不能断言“谁引用了谁”，只能写“可能存在承接关系，需进一步核验”。
- 创新点只能说“在当前文献范围内较少发现”，不能说“首次提出”“无人研究”“绝对创新”。

## 项目结构

```text
chinese-academic-paper-skill/
├─ .github/ISSUE_TEMPLATE/             # 使用问题、导入格式、功能建议模板
├─ chinese-academic-paper-assistant/   # Codex skill 主体
│  ├─ SKILL.md
│  ├─ references/                      # 工作流、数据库边界、写作规则
│  ├─ scripts/                         # BibTeX/RIS 解析、矩阵校验等脚本
│  └─ assets/                          # 文献矩阵和输出模板
├─ docs/                               # 新手文档和提示词模板
├─ examples/                           # 示例输入与输出
├─ SHARE.md                            # 推广固定入口
├─ CHANGELOG.md                        # 版本更新记录
├─ CONTRIBUTING.md                     # 贡献指南
└─ LICENSE                             # MIT License
```

## 版本路线

- v0.1：文献导入、公开来源补充、文献矩阵、研究脉络、创新点、去 AI 化综述、基础期刊推荐。
- v0.1.3：Zotero/BibTeX/RIS 导入教程和示例工作流。
- v0.1.4：复制即用提示词模板和新手使用文档。
- v0.1.5：本科论文、硕士论文、期刊论文三种输出模式。
- v0.1.6：研究领域知识地图，输出核心主张、支持柱、争议区、前沿问题和初学者必读论文。
- v0.1.7：首批文献主张表、共同假设分组、矛盾标记和宏观背景图；适用于用户上传文献和公开来源候选文献。
- v0.1.8：创新点相近论文与方法论，为每个创新点标记最接近论文、相近之处、可推进空间和推荐方法论。
- v0.1.9：完整中文工作流演示、适用/不适用场景、最小可用提示词和防编造文献规则强化。
- v0.1.10：完整版文字式文献综述，在完整流程最后逐篇覆盖所有已找到并进入矩阵或候选表的文献。
- v0.1.11：开源版一键综述辅助模式，上传 8 篇以上文献或由 skill 规划候选文献后，输出综述草稿要求、核验提醒和文献覆盖简表。
- Unreleased：新增不安装也能先用的提示词版入口，以及“没有文献 → 8 篇真实公开候选 → 待核验综述”的完整案例。
- v0.2：PDF 文献信息读取原型、授权万方接口接入规划、中文元数据核验增强、批量文献分析和用户反馈优先级排序。
- v0.3+：更完善的 CNKI/VIP/学校图书馆导入流程、更多导出格式、期刊官网公开元数据处理。

## 后续计划

如果你希望支持万方接口接入、批量文献分析、课题组定制版或机构版，可以通过 GitHub Issues 提需求。后续商业版会优先考虑：

- 万方授权接口
- 批量文献矩阵
- 矛盾强度评分和动态可视化知识图谱
- 引用关系增强分析
- 创新点评分
- 自动相似度评分、方法论适配评分和创新点可视化关系图
- 中文期刊推荐报告
- Word 文档导出

### 反馈入口

如果你正在使用免费网页版，也欢迎直接提交需求。现在最希望收集三类反馈：PDF 文献信息读取、Word 导出、GB/T 7714 参考文献格式。

- [我想要网页直接生成综述 / PDF 解析 / Word 导出](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/new?template=feature-request.md)
- [PDF 文献信息读取 Roadmap Issue](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/3)
- [Word 文档导出 Roadmap Issue](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/4)
- [GB/T 7714 参考文献格式 Roadmap Issue](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/5)
- [v0.2 PDF 文献信息读取设计文档](./docs/v0.2-pdf-literature-extraction.md)
- [我遇到了使用问题](https://github.com/qinky1234-sys/chinese-academic-paper-skill/issues/new?template=usage-question.md)

## 社区推广

如果你想把这个项目推荐到中文 GitHub 社区，可以使用这份推广素材：

- [GitHub 中文社区推广帖](./promotion/github-chinese-community-post.md)
- [推广执行清单](./promotion/promotion-checklist.md)

## 参与贡献

欢迎提交中文数据库导出格式、Zotero/BibTeX/RIS 示例、提示词模板、文档修正和脚本改进。提交前请阅读 [贡献指南](./CONTRIBUTING.md)。如果你只是想反馈问题，可以直接使用 GitHub Issues 中的模板。

本项目使用 [MIT License](./LICENSE)。
